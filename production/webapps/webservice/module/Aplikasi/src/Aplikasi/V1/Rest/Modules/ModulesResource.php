			...
			if(isset($params['TIPE'])) {
				if($params['TIPE'] == 1) {		
					if(isset($params['GROUP_PENGGUNA'])) {
						$columns = array('*', 'checked' => new \Zend\Db\Sql\Expression("aplikasi.isSelectedGroupPenggunaAksesModule(".$params['GROUP_PENGGUNA'].", modules.ID)"));						
						unset($params['GROUP_PENGGUNA']);
					}
				} else {					
					if(isset($params['GROUP_PENGGUNA']) && isset($params['PENGGUNA'])) {
						$columns = array('*', 'checked' => new \Zend\Db\Sql\Expression("aplikasi.isSelectedPenggunaAkses(".$params['PENGGUNA'].", modules.ID, ".$params['GROUP_PENGGUNA'].")"));						
